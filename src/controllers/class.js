...
const { classProjection } = require('../models/Projections_Schema')
...
const create = async (req, res, next) => {
  try {
    let newData = new Class({ ...req.body })
...
const findOneById = async (req, res, next) => {
  try {
    ...
    const data = await Class.findOne(query, classProjection)
...
const findAll = async (req, res, next) => {
  try {
    ...
    const data = await Class.find(query, classProjection, options)
      .limit(limit)
      .skip(limit * (page - 1))
